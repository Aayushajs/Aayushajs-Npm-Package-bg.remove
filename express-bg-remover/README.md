...
A production-ready, lightweight Express middleware for removing image backgrounds using an external API (like Hugging Face spaces). By default, it natively uses a built-in Hugging Face endpoint.
...
Here is a complete, working example of how to configure your Express route using `multer` and our background removal middleware. The Hugging Face API URL is already built-in, so you don't even need to pass it!
...
|---|---|---|---|
| `apiUrl` | `string` | `"https://DevelopmentT-background-remover.hf.space/remove-bg"` | (Optional) Endpoint for POSTing the image. |
